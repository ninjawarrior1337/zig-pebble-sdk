...
fn pebble_header_fixup(b: *std.Build, io: std.Io, pebble_include_path: []const u8) []const u8 {
    var pebble_header = std.Io.Dir.cwd().readFileAlloc(io, b.pathJoin(&.{ pebble_include_path, "pebble.h" }), b.allocator, .limited(1 << 20)) catch @panic("OOM");
...
fn pebble_linker_script_template(b: *std.Build, io: std.Io, pebble_linker_script_template_path: []const u8, platform: PebblePlatform) []const u8 {
    const linker_script = std.Io.Dir.cwd().readFileAlloc(io, pebble_linker_script_template_path, b.allocator, .limited(1 << 20)) catch @panic("OOM");
...
fn pebble_appids_zig_template(b: *std.Build, platform: PebblePlatform, metadata: PebbleAppMetadata) []const u8 {
    ...
    // Collect media
    var resource_ids: std.ArrayList(u8) = .empty;
    var id: usize = 1;
...
pub fn addPebbleApplication(b: *std.Build, options: PebbleApplicationOptions) void {
    ...
    var pebble_app_artifacts: std.ArrayList(struct {
    ...
    for (options.pebble.targetPlatforms) |platform| {
        ...
        const linker_script = linker_script_step.add(b.fmt("{s}.gen.ld", .{options.name}), pebble_linker_script_template(b, io, paths.pebble_linker_script_template_path, platform));
        ...
        pebble_app_artifacts.append(b.allocator, .{
            .platform = platform,
